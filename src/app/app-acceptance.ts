...
export const appTransferMode: ToolcraftTransferMode = {
  animationIntent: {
    loopDuration: {
      evidence:
        "ffprobe reports a 1.4667s duration for the provided reference video 63dba3fdfa6d75604436eae5ebde4d4f_720w.mp4; the product loop rounds it to 1.5s.",
      seconds: 1.5,
      source: "reference",
    },
    mode: "timeline-playback",
    ...
    acceptanceMapping: [
      {
        ...
        behavior:
          "Uploaded images play as an equal-duration frame sequence over one seamless forward-only loop.",
        frameIds: ["f01", "f02", "f06"],
      },
      {
        acceptanceId: "control.frames.transition",
        behavior:
          "Frame changes are hard cuts by default, matching every observed reference transition.",
        frameIds: ["f02", "f03", "f04"],
      },
      {
        acceptanceId: "control.frames.images",
        behavior:
          "The frame order comes from the ordered source image set; each shot is a distinct source image.",
        frameIds: ["f01", "f03", "f05"],
      },
      {
        acceptanceId: "control.background.mode",
        behavior:
          "Each frame subject is presented centered on a styled backdrop layer behind it.",
        frameIds: ["f01", "f04", "f06"],
      },
    ],
    behaviorDecomposition:
      "The reference decomposes into an ordered source-image set, uniform per-frame timing across the loop, hard-cut transitions between frames, a centered subject composed over a backdrop layer, and a seamless forward-only loop where the last shot cuts back to the first.",
    extractionEvidence:
      "Extracted six frames with ffmpeg -vf fps=4 from the provided mp4 and reviewed each extracted frame image before implementation.",
    referenceLocation: "~/Downloads/63dba3fdfa6d75604436eae5ebde4d4f_720w.mp4",
    storyboard: [
      {
        behaviorObservation:
          "The sequence starts on a full shot; the subject (woven label) is centered on a dark textured backdrop.",
        frameId: "f01",
        frameSource: "ffmpeg-extracted frames/f01.png (fps=4)",
        timeSeconds: 0,
        visualObservation:
          "A silver woven brand label centered on near-black textile filling the canvas.",
      },
      {
        behaviorObservation:
          "A hard cut replaced the entire shot; no partial blend of the previous frame remains.",
        frameId: "f02",
        frameSource: "ffmpeg-extracted frames/f02.png (fps=4)",
        timeSeconds: 0.25,
        visualObservation: "A different fabric close-up with new crop and lighting.",
      },
      {
        behaviorObservation:
          "Another hard cut; the subject stays centered while material and crop change.",
        frameId: "f03",
        frameSource: "ffmpeg-extracted frames/f03.png (fps=4)",
        timeSeconds: 0.5,
        visualObservation:
          "A black EVERBLOOM label centered inside a fuzzy knit collar.",
      },
      {
        behaviorObservation:
          "Uniform cadence continues: each shot occupies an equal share of the loop.",
        frameId: "f04",
        frameSource: "ffmpeg-extracted frames/f04.png (fps=4)",
        timeSeconds: 0.75,
        visualObservation: "A new material close-up with the brand mark centered.",
      },
      {
        behaviorObservation:
          "The subject remains the visual anchor; only the surrounding backdrop texture changes per shot.",
        frameId: "f05",
        frameSource: "ffmpeg-extracted frames/f05.png (fps=4)",
        timeSeconds: 1,
        visualObservation: "Another centered brand-detail shot on textured cloth.",
      },
      {
        behaviorObservation:
          "The final shot cuts back to the first frame when the video loops, with no reverse or mirror playback.",
        frameId: "f06",
        frameSource: "ffmpeg-extracted frames/f06.png (fps=4)",
        timeSeconds: 1.25,
        visualObservation:
          "A white EVERBLOOM card centered on a textured black-and-white throw.",
      },
    ],
    transitionAnalysis: [
      {
        behaviorDelta:
          "Between f01 and f02 the entire shot is replaced in one frame: a hard cut with no dissolve, wipe, or camera motion; the centered-subject composition persists.",
        fromFrameId: "f01",
        id: "f01-f02",
        toFrameId: "f02",
      },
      {
        behaviorDelta:
          "Between f02 and f03 another hard cut retargets to a new material; timing between cuts is uniform, implying equal per-frame slots across the loop duration.",
        fromFrameId: "f02",
        id: "f02-f03",
        toFrameId: "f03",
      },
      {
        behaviorDelta:
          "Between f03 and f04 the cut keeps the subject anchor centered while the backdrop texture changes, showing the backdrop is a separate presentation layer.",
        fromFrameId: "f03",
        id: "f03-f04",
        toFrameId: "f04",
      },
      {
        behaviorDelta:
          "Between f06 and f01 the loop wraps forward to the first shot with a hard cut; playback direction never reverses, so the sequence loops seamlessly forward-only.",
        fromFrameId: "f06",
        id: "f06-f01",
        toFrameId: "f01",
      },
...
export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    entity: "Source frames",
    groupingReason:
      "The multi-image uploader is the source-material stage of the frame sequence; the runtime renders the sortable upload grid as its own standalone block.",
    splitReason:
      "Importing the source images is the source-material workflow stage; tuning how frames hand over is a separate sequencing stage rendered after it.",
    targets: ["frames.images"],
    title: "Section images",
    workflowStage: "source",
  },
  {
    entity: "Frame sequencing",
    groupingReason:
      "The transition selector defines how one frame hands over to the next during the loop.",
    splitReason:
      "Sequencing behavior is tuned after the source images are imported; the runtime renders the standalone upload grid separately from this stage.",
    targets: ["frames.transition"],
    title: "Frames",
    workflowStage: "sequencing",
  },
  {
    entity: "Frame presentation",
    groupingReason:
      "These controls style how each section image is composed on the canvas: its size, corner rounding, and drop shadow.",
    targets: ["frame.scale", "frame.cornerRadius", "frame.shadow"],
    title: "Frame Style",
  },
  {
    entity: "Output background",
    groupingReason:
      "These controls configure the backdrop layer behind every frame: its type (solid, gradient, or image), the type-specific fill, and whether PNG export includes it.",
    targets: [
      "background.mode",
  ...
  },
  {
    ...
    groupingReason:
      "Format and resolution are one compact export workflow pair for still PNG/JPG output.",
  ...
  },
  {
    ...
    groupingReason:
      "Format and resolution are one compact export workflow pair for animated video output.",
...
export const appAcceptance: readonly ToolcraftComponentAcceptance[] = [
  {
    ...
    browser: true,
    browserTestName:
      "browser: section images upload reorder rotate flip clear and reset drive the frame sequence",
    componentType: "fileDrop",
    evidence: "media-lifecycle",
    expectedObservable:
      "Uploading section images renders them as frames in runtime media order; reorder changes which frame renders first because the renderer consumes runtime media order; rotate 90° and flip update mediaAssets[].transform and the rendered output consumes the transform; clear/remove empties the canvas frame; global Reset controls removes uploaded source media since no default asset exists.",
    fixture: "Two SVG section screenshots with distinct solid fills.",
    id: "control.frames.images",
    kind: "control",
    target: "frames.images",
    userAction:
      "Upload two images, drag thumbnails to reorder, apply 90° rotate and Flip H to the selected image, remove one image, then click Reset controls to remove the uploaded source media.",
  },
  {
    automated: true,
    automatedTestName: "transition mode switches between cut and crossfade frame mixing",
    browser: true,
    browserTestName: "browser: transition select switches cut and crossfade output",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "With Cut, mid-slot boundaries render exactly one opaque frame; with Crossfade, the end of each slot blends the next frame over the current one and the canvas pixels change.",
    fixture: "Two uploaded section images and a scrubbed timeline position.",
    id: "control.frames.transition",
    kind: "control",
    optionCoverage: ["cut", "crossfade"],
    target: "frames.transition",
    userAction: "Choose Crossfade, scrub into a slot boundary, then choose Cut.",
    },
  {
    automated: true,
    automatedTestName: "frame scale changes the drawn frame footprint",
    browser: true,
    browserTestName: "browser: frame scale slider updates the composed frame live during drag",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging the Scale thumb updates runtime frame.scale and shrinks or grows the rendered frame while the drag is still in progress.",
    fixture: "One uploaded section image on a solid background.",
    id: "control.frame.scale",
    kind: "control",
    target: "frame.scale",
    userAction: "Drag the Scale slider thumb from 70% toward 30% and observe the canvas during the drag.",
  },
  {
    automated: true,
    automatedTestName: "corner radius rounds the frame corners",
    browser: true,
    browserTestName: "browser: corner radius slider rounds the frame corners",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging Corner radius updates runtime frame.cornerRadius and visibly rounds the composed frame corners during the drag.",
    fixture: "One uploaded section image with a contrasting background.",
    id: "control.frame.cornerRadius",
    kind: "control",
    target: "frame.cornerRadius",
    userAction: "Drag the Corner radius slider from 16px to 80px.",
  },
  {
    automated: true,
    automatedTestName: "frame shadow toggles the drop shadow pass",
    browser: true,
    browserTestName: "browser: frame shadow switch toggles the drop shadow",
    componentType: "switch",
    evidence: "product-output",
    expectedObservable:
      "Turning Shadow off removes the soft drop shadow behind the frame and the rendered pixels around the frame edge change.",
    fixture: "One uploaded section image scaled below the canvas bounds.",
    id: "control.frame.shadow",
    kind: "control",
    target: "frame.shadow",
    userAction: "Toggle the Shadow switch off and back on.",
  },
  {
    automated: true,
    ...
    browser: true,
    browserTestName:
      "browser: background mode select switches solid gradient and image backdrops",
    componentType: "select",
    evidence: "product-output",
    expectedObservable:
      "Choosing Solid fills the backdrop with background.color, Gradient renders the gradient stops, and Image renders the uploaded backdrop cover/cropped; inactive branch controls (color, gradient, backdrop upload) are hidden with visibleWhen and their values are preserved when switching away and back; with Include off, any Type exports a transparent PNG (alpha), the preview hides the product background, and video export keeps the background.",
    fixture:
      "One uploaded section image plus one uploaded backdrop image; distinct solid and gradient colors.",
    id: "control.background.mode",
    kind: "control",
    optionCoverage: ["solid", "gradient", "image"],
    target: "background.mode",
    userAction:
      "Switch Type between Solid, Gradient, and Image and observe the backdrop and visible controls.",
  },
  {
    automated: true,
    ...
    browser: true,
    browserTestName:
      "browser: include background toggle controls preview and png transparency while video keeps background",
    componentType: "switch",
    evidence: "exported-bytes",
    expectedObservable:
      "Turning Include off hides the product background in the live preview through shouldIncludeToolcraftPreviewBackground(state) and exports a transparent PNG (alpha pixels outside the frame); turning it on includes the current background color in the PNG; video export keeps the background either way.",
    fixture: "One uploaded section image on a saturated solid background.",
    id: "control.export.includeBackground",
    kind: "control",
    target: "export.includeBackground",
    userAction:
      "Toggle Include off, click Export PNG, toggle Include on, export again, then export video with Include off.",
  },
  {
    automated: true,
    automatedTestName: "background color fills the solid backdrop",
    browser: true,
    browserTestName: "browser: background color updates the solid backdrop",
    componentType: "color",
    evidence: "product-output",
    expectedObservable:
      "Changing the background color hex in Solid mode changes the rendered backdrop pixels; the color control is visible only while Type is Solid and hidden when the mode switches to Gradient or Image.",
    fixture: "Solid background mode with no backdrop image.",
    id: "control.background.color",
    kind: "control",
    target: "background.color",
    userAction: "Open the background color control and enter a new hex value.",
  },
  {
    automated: true,
    automatedTestName: "background gradient renders type angle and stops",
    browser: true,
    browserTestName: "browser: background gradient edits type angle and stops",
    componentType: "gradient",
    controlPartCoverage: "all-visible-parts",
    evidence: "product-output",
    expectedObservable:
      "Changing gradient type (linear/radial), angle, a stop position, a stop color, and a stop opacity each change the rendered backdrop pixels in Gradient mode; the gradient control is visible only while Type is Gradient and hidden in other modes.",
    fixture: "Gradient background mode with two high-contrast stops.",
    id: "control.background.gradient",
    kind: "control",
    target: "background.gradient",
    userAction:
      "Switch gradient type, change the angle, drag a stop position, change a stop color, and change a stop opacity.",
  },
  {
    automated: true,
    ...
    browser: true,
    browserTestName:
      "browser: backdrop image upload rotate flip clear and reset update the background",
    componentType: "fileDrop",
    evidence: "media-lifecycle",
    expectedObservable:
      "Uploading a backdrop image in Image mode renders it cover/cropped behind the frame; 90° rotate and flip update mediaAssets[].transform and the rendered backdrop consumes the transform; clear/remove falls back to the solid color; global Reset controls removes the uploaded backdrop since no default asset exists; the uploader is visible only while Type is Image and hidden in other modes.",
    fixture: "One backdrop image with an aspect ratio different from the canvas.",
    id: "control.background.image",
    kind: "control",
    target: "background.image",
    userAction:
      "Switch Type to Image, upload a backdrop, apply 90° rotate and Flip V, remove it, then click Reset controls.",
  },
  {
    automated: true,
    automatedTestName: "background blur softens the backdrop image",
    browser: true,
    browserTestName: "browser: background blur slider softens the backdrop image",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Dragging Blur in Image mode applies a canvas blur to the backdrop image so the rendered backdrop pixels change (sharp edges become soft); the Blur slider is visible only while Type is Image and hidden in Solid and Gradient modes.",
    fixture: "Image background mode with a high-contrast backdrop image behind the frame.",
    id: "control.background.blur",
    kind: "control",
    target: "background.blur",
    userAction: "Switch Type to Image, upload a backdrop, then drag the Blur slider up.",
  },
  {
    automated: true,
    ...
    browser: true,
    browserTestName:
      "browser: image export format and resolution change exported bytes",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "PNG format exports image/png bytes and JPG exports image/jpeg bytes with the background always included for JPG.",
    fixture: "One uploaded section image.",
    id: "control.export.image.format",
    kind: "control",
    optionCoverage: ["png", "jpg"],
    target: "export.image.format",
    userAction: "Choose PNG, export, choose JPG, export again.",
  },
  {
    automated: true,
    ...
    evidence: "exported-bytes",
    expectedObservable:
      "2K exports a 2048px long-edge image, 4K exports 4096px, and 8K exports 8192px; decoded exported bytes prove the actual pixel dimensions changed.",
    fixture: "Default 1920x1080 canvas with one uploaded section image.",
    id: "control.export.image.resolution",
    kind: "control",
    optionCoverage: ["2k", "4k", "8k"],
    target: "export.image.resolution",
    userAction: "Choose 2K and export, then choose 4K and export again.",
  },
  {
    automated: true,
    ...
    evidence: "exported-bytes",
    expectedObservable:
      "MP4 exports through MediaRecorder.isTypeSupported with a safe fallback to WebM when mp4 recording is unsupported; WebM exports video/webm bytes; the exported blob loads as a video whose metadata duration matches the runtime timeline duration.",
    fixture: "Two uploaded section images and the default 1.5s timeline.",
    id: "control.export.video.format",
    kind: "control",
    optionCoverage: ["mp4", "webm"],
    target: "export.video.format",
    userAction: "Choose MP4 and export video, then choose WebM and export again.",
  },
  {
    automated: true,
    ...
    evidence: "exported-bytes",
    expectedObservable:
      "Current exports at the current canvas/output size with even encoder-safe rounding; 4K fits inside 3840x2160 through getToolcraftVideoExportSize, preserves aspect ratio, and returns even dimensions.",
    fixture: "Default 1920x1080 canvas with one uploaded section image.",
    id: "control.export.video.resolution",
    kind: "control",
    optionCoverage: ["current", "4k"],
    target: "export.video.resolution",
    userAction: "Choose Current and export video, then choose 4K and export again.",
  },
  {
    automated: true,
    automatedTestName: "export actions deliver gif video and png output",
    browser: true,
    browserTestName:
      "browser: export actions deliver gif video and png output with a pending progress indicator",
    componentType: "panelActions",
    actionCoverage: ["export-video", "export-png", "export-gif"],
    evidence: "exported-bytes",
    expectedObservable:
      "Export GIF downloads image/gif bytes whose frame count and delays cover the timeline duration; Export Video downloads a real video blob; Export PNG downloads image bytes; each handler returns a real Promise so the sticky footer top accent indicator is visible while pending, advances with reportProgress, and hides after settle.",
    fixture: "Two uploaded section images on the default canvas.",
    id: "control.export.actions",
    kind: "control",
    target: "export.actions",
    userAction: "Click Export GIF, Export Video, and Export PNG in the sticky footer.",
  },
  {
    automated: true,
    ...
    browser: true,
    browserTestName:
      "browser: timeline playback scrub duration and loop drive the frame cycle",
    componentType: "timeline",
    evidence: "timeline-output",
    expectedObservable:
      "Play advances the frame sequence and Pause freezes it; scrubbing selects the frame for that time; editing the timeline duration remaps one full frame cycle to state.timeline.durationSeconds via getToolcraftTimelineLoopProgress; after changing the duration the loop still advances forward-only in one direction with no mirror, no yo-yo, no ping-pong, and no reverse fallback, and the first and last frames stitch with no visible jump at the wrap; with looping off, pressing Play at the end restarts from 0.",
    fixture: "Three uploaded section images with distinct fills.",
    id: "runtime.timeline.playback",
    kind: "runtime",
    timelineCoverage: "playback",
    timelinePlaybackCoverage: "all-playback-behavior",
    userAction:
      "Press Play, Pause, scrub the timeline, edit the timeline duration, toggle loop, and compare rendered frames at 0, midpoint, end minus epsilon, and the wrapped first frame.",
  },
  {
    automated: true,
    automatedTestName: "render scale slider is a discrete control",
    browser: true,
    browserTestName: "browser: resolution scale discrete slider changes canvas backing pixels",
    componentType: "slider",
    evidence: "viewport-side-effect",
    expectedObservable:
      "The runtime Resolution scale slider renders the discrete Toolcraft variant with tick markers, drags smoothly, and raising it increases the product canvas backing pixels without changing the CSS output size.",
    fixture: "One uploaded section image on the default 1920x1080 canvas.",
    id: "runtime.canvas.renderScale",
    kind: "control",
    target: "canvas.renderScale",
    userAction: "Drag the Resolution scale slider and read the canvas backing pixels.",
  },
  {
    automated: true,
    ...
    browser: true,
    browserTestName:
      "browser: uploading a different aspect image keeps canvas size and covers within bounds",
    componentType: "canvas",
    evidence: "product-output",
    expectedObservable:
      "Uploading a section image with a different aspect ratio keeps the current canvas.size unchanged (1920x1080 default stays), the Setup Aspect ratio / Canvas width / Canvas height controls stay visible, and the image renders contained inside the current canvas bounds at frame.scale.",
    fixture: "A portrait 678x904 image on the default 1920x1080 canvas.",
    id: "runtime.canvas.uploadSizing",
    kind: "runtime",
    userAction:
      "Upload a portrait image and read the canvas size controls and rendered output.",
  },
];
...
const neutralToolcraftTransferMode: ToolcraftTransferMode = {
...
export function validateToolcraftAcceptanceCoverage(
  ...
  explicitTransferMode?: ToolcraftTransferMode,
  explicitSectionInventory?: readonly ToolcraftControlSectionInventoryEntry[],
): string[] {
  // Fixture schemas validated by template unit tests must not inherit the real
  // app's transfer mode or section inventory through default arguments.
  const isRealAppSchema = schema === appSchema;
  const transferMode =
    explicitTransferMode ?? (isRealAppSchema ? appTransferMode : neutralToolcraftTransferMode);
  const sectionInventory =
    explicitSectionInventory ?? (isRealAppSchema ? starterControlSectionInventory : []);
  const errors: string[] = [];
