...
describe("Toolcraft template app acceptance coverage", () => {
  ...
  it("requires explicit runtime acceptance before locking canvas output size", () => {
    ...
    expect(validateToolcraftAcceptanceCoverage(fixedOutputSchema, [])).toEqual(
      expect.arrayContaining([
  ...
  it("requires intrinsic media sizing acceptance for upload apps that let media own canvas size", () => {
    ...
    expect(validateToolcraftAcceptanceCoverage(intrinsicUploadSchema, [])).toEqual(
      expect.arrayContaining([
  ...
  it("declares the new Toolcraft assembly mode with a playback loop and video reference study", () => {
    expect(appTransferMode.mode).toBe("new-toolcraft-app");
    expect(appTransferMode.animationIntent).toMatchObject({
      loopDuration: { seconds: 1.5, source: "reference" },
      mode: "timeline-playback",
    });
    expect(appTransferMode.videoReferenceStudy?.storyboard.length).toBeGreaterThanOrEqual(4);
    expect(
      appTransferMode.videoReferenceStudy?.transitionAnalysis.length,
