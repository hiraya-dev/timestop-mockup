...
export async function dragToolcraftSliderByLabel(
  ...
  // Scroll the slider clear of the sticky footer/other panels so the thumb is
  // not occluded when the pointer interaction starts.
  ...
  // The Base UI slider only reacts to drags that begin on the thumb, so start
  // the pointer interaction from the current thumb position when available.
  const thumb = field.locator('[data-slot="slider-thumb"]').first();
  const thumbBox = (await thumb.count()) > 0 ? await thumb.boundingBox() : null;
  const startX = thumbBox ? thumbBox.x + thumbBox.width / 2 : box.x + box.width * 0.15;
  const startY = thumbBox ? thumbBox.y + thumbBox.height / 2 : box.y + box.height / 2;
